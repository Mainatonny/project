...
class MEAformer(nn.Module):
    def __init__(self, kgs, args):
        ...
        self.multi_loss_layer = CustomMultiLossLayer(loss_num=5)  # 6
    ...
    def forward(self, batch):
        gph_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, hidden_states = self.joint_emb_generat(only_joint=False)
        gph_emb_hid, rel_emb_hid, att_emb_hid, name_emb_hid, char_emb_hid, joint_emb_hid = self.generate_hidden_emb(hidden_states)
        if self.args.replay:
        ...
        in_loss = self.inner_view_loss(gph_emb, rel_emb, att_emb, name_emb, char_emb, batch)
    ...
    def generate_hidden_emb(self, hidden):
        ...
        if hidden.shape[1] >= 5:
            name_emb = F.normalize(hidden[:, 3, :].squeeze(1))
            char_emb = F.normalize(hidden[:, 4, :].squeeze(1))
            joint_emb = torch.cat([gph_emb, rel_emb, att_emb, name_emb, char_emb], dim=1)
        else:
            name_emb, char_emb = None, None
            joint_emb = torch.cat([gph_emb, rel_emb, att_emb], dim=1)

        return gph_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb

    def inner_view_loss(self, gph_emb, rel_emb, att_emb, name_emb, char_emb, train_ill):
        ...
        total_loss = self.multi_loss_layer([loss_GCN, loss_rel, loss_att, loss_name, loss_char])
        return total_loss

    # --------- necessary ---------------

    def joint_emb_generat(self, only_joint=True):
        gph_emb, rel_emb, att_emb, \
        ...
        if only_joint:
            ...
            return gph_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, hidden_states
