...
class TransformerXLAttention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, dropout=0.1):
        ...
        self.relative_position_bias = nn.Parameter(torch.randn(num_attention_heads, self.attention_head_size))
    ...
    def forward(self, hidden_states, prev_hidden_states=None):
        # Project inputs to query, key, and value
        query = self.transpose_for_scores(self.query(hidden_states))
        key = self.transpose_for_scores(self.key(hidden_states if prev_hidden_states is None else prev_hidden_states))
        value = self.transpose_for_scores(self.value(hidden_states if prev_hidden_states is None else prev_hidden_states))
        ...
        relative_position_bias = self.relative_position_bias.unsqueeze(0).unsqueeze(0)
...
class MformerFusion(nn.Module):
    def __init__(self, args, modal_num, with_weight=1):
        ...
        self.fusion_layer = nn.ModuleList(
            [TransformerXLAttention(args.hidden_size, args.num_attention_heads) for _ in range(args.num_hidden_layers)])
...
class MultiModalEncoder(nn.Module):
    ...
    def __init__(self, args,
        ...
        self.ftdea = FTDEA(e_hidden=300, r_hidden=200, t_hidden=200)
    ...
    def forward(self,
        ...
        if self.args.w_gcn:
            edge_index = torch.randint(0, self.ENT_NUM, (2, self.ENT_NUM), device='cuda')
            rel = torch.argmax(rel_features, dim=1)
            rel_counts = torch.bincount(rel, minlength=rel_features.size(1))
            rel_size = rel_counts[rel].clone()

            otiea_feature = self.ftdea(self.entity_emb(input_idx), edge_index, rel, edge_index, rel_size)
            gph_emb = self.cross_graph_model(self.entity_emb(otiea_feature.long()), adj)
        ...
        joint_emb, hidden_states, weight_norm = self.fusion([att_emb, rel_emb, gph_emb, name_emb, char_emb])

        return gph_emb, rel_emb, att_emb, name_emb, char_emb, joint_emb, hidden_states, weight_norm
